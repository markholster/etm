...
De ETM IBM MQ processor is te vinden in de directory <ETM_UNZIP_DIR>/components/etm-processor-ibmmq-{etm_version} en te verplaatsen naar elke willekeurige locatie. De configuratie
van de processor is terug te vinden in de <ETM_UNZIP_DIR>/components/etm-processor-ibmmq-{etm_version}/etc. Het bestand etm.yml is het primaire configuratie bestand. In dit bestand 
plaatst u uw MQ connectie parameters. Het bestand is voorzien van commentaarregels per in te richten optie. 
...
de Java MQ libraries te plaatsen in de <ETM_UNZIP_DIR>/components/etm-processor-ibmmq-{etm_version}/lib directory. Het volstaat om het bestand 'com.ibm.mq.allclient.jar' te kopieren naar 
deze lib directory.
...
<ETM_UNZIP_DIR>/components/etm-processor-ibmmq-{etm_version}/bin. Op een unix omgeving zal de ETM IBM MQ processor geen output geven op de console, maar enkel loggen naar de log
files die terug te vinden zijn in de directory <ETM_UNZIP_DIR>/components/etm-processor-ibmmq-{etm_version}/logs. Indien u fouten wilt opsporen kan ETM IBM MQ processor gestart worden 
in de 'console' modus waardoor alle output ook naar de console gelogd wordt. Deze modus kan gestart worden door het start script uit te voeren met de parameter 'console'.
