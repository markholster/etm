...
maar voor optimale situatie zullen deze clusters zo geconfigureerd moeten worden dat deze ideaal zijn voor uw wensen. 
...
=== ETM gui
De ETM gui is het ETM component waarmee u uw events kunt monitoren, doorzoeken en het ETM cluster kunt beheren en administreren. Om de ETM gui te gebruiken dient u de component 
...
specifieke eigenschappen zoals d queue naam uit te lezen. Dit zijn de waardes die beschreven als "${<waarde>}" in de ejb-jar.xml voorkomen. Uiteraard hoeft u hier geen 
gebruik te maken van system properties, maar kunt u hier ook gewoon harde waardes gebruiken.
...
----
De tweede regel is niet strikt noodzakelijk, maar kan u helpen bij de volgende stap.
...
Wijzign hiervoor de naam van het bestand WEB-INF/jboss-ejb3-ra-adapter-example.xml naar WEB-INF/jboss-ejb3.xml, en pas de inhoud zo aan dat hier de gewenste resource adapter
...
code hierboven genoemd wel noodzakelijk.

Uw MDB is nu klaar voor gebruik. Een beschrijving over de inhoud waaraan de JMS Message moet voldoen die de MDB uitleest is te vinden in de gebruikershandleiding.

=== ETM data behoud
Om event na verloop van tijd uit ETM te verwijderen dient u de component etm-scheduler-retention-{etm_version}.warte deployen in Wildfly. Voor dit component is geen overige 
configuratie nodig. Een beschrijving voor het instellen van de bewaartijd van events is te vinden in de gebruikershandleiding.
