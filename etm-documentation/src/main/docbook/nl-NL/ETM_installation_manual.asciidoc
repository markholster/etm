...
=== CPU
De CPU requirements van ETM doen er minder toe dan het geheugen of de hard disks. Het is waarsschijnlijk dat uw cluster eerder tegen de 
...
IMPORTANT: ETM is een op JEE gebaseerde Java applicatie die binnen Wildfly gedeployed kan worden. ETM heeft een verbinding nodig met Elasticsearcg om
...
handleiding samengevat worden. Dit hoofdstuk kan als voorbeeld genomen worden voor het opzetten van uw Wildfly en Elasticsearcg clusters, 
...
In verband met data replicatie wordt er voor het gebruik van ETM in een productie omgeving een minimum van 2 Elasticsearch installaties geadviseerd. Hiervoor kan
het <<standalone-elasticsearch, Standalone Elasticsearch>> hoofdstuk worden uitgevoerd op verschillende machines. De instanties zullen elkaar automatisch vinden
en een cluster vormen.
...
|Key|Value
|etm.cluster.name|<de naam van uw ETM cluster>
|etm.cluster.addresses|Een optionele komma gescheiden lijst van Elasticsearch hosts in de vorm van ip-adres:poortnr. Gebruik dit alleen als uw firewall de auto-discovery functionaliteit van Elasticsearch blokkeert.
|=======================
