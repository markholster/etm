...
event terug te vinden is in de zoekresultaten. Om te voorkomen dat berichten bij een lage toevoer langdurig niet beschikbaar zijn wordt een batch minstens 1 maal per dertig 
...
Onder de tab 'ETM' vindt u in de linker kolom vier configuratie opties die van belang zijn bij het verwerken van events:
...
'Persisting handlers' ingesteld en de 'Persisting bulk size' staat op 100, dan zullen er op elk moment 5 batches van maximaal 100 events worden gebruikt. 
...
Elke shard heeft zijn eigen set resources (denk aan file handles etc) nodig op een systeem. Als vuistregel geldt de volgende formule:
...
image:usage/data-duplication.png["Data spreiding invoer", link="usage/data-duplication.png"]

De instelling 'Write Consistency' bepaalt wanneer een schrijf- of delete operatie in het cluster afgerond is. De instelling heeft 3 mogelijk opties:

* ONE, een operatie is afgerond zodra 1 van de nodes de actie heeft afgehandeld.
* QUORUM, een operatie is afgerond zodra <aantal replica's> / 2 + 1 nodes de actie hebben afgehandeld. Indien het aantal replica's op 1 is ingesteld is de actie afgerond
 indien 1 van de nodes de actie uitgevoerd heeft.
* ALL, een operatie is afgerond zodra alle replica's de actie hebben afgehandeld. 
...
=== Zoekresultaten
Nadat u uw event hebt teruggevonden kunt u de inhoud bekijken door op de eventnaam te klikken. Allereerst vindt u hier enkele eigenschappen van het event terug, maar ook de inhoud
...
|[[error-100001]]100001|Ongeldige licentie sleutel|De ingevulde licentie sleutel is niet geldig. Neem contact op met Jecstar Innovation support.
|[[error-100002]]100002|De licentie sleutel is verlopen|De huidige licentie sleutel is niet meer gelidg. Neem contact op met Jecstar Innovation support.
...
|[[error-100007]]100007|Ongeldige expressie parser|De expressie parser wordt niet herkend. Neem contact op met Jecstar Innovation support. 
|=======================
