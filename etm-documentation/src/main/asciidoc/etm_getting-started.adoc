...
If your organization not already has an Elasticsearch cluster installed it is time to install it now. {etm} needs an Elasticsearch cluster formed of nodes with version 5.3 or compatible. The installation of Elasticsearch is covert at the link:https://www.elastic.co/guide/en/elasticsearch/reference/5.3/\_installation.html[Elastic.co\'s website]. 
Don\'t forget to read the link:https://www.elastic.co/guide/en/elasticsearch/reference/5.3/important-settings.html[important configurations] and link:https://www.elastic.co/guide/en/elasticsearch/reference/5.3/system-config.html[important system configurations] pages! 
