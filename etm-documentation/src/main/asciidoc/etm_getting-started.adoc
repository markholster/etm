...
license. If your not comfortable getting an Elasticsearch subscription the ''oss'' version is the one to pick. It might be a little tricky to find but it can be downloaded by adding ''-oss'' before the version tag in your download link. For example the zipped version can be
downloaded from link:https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-oss-{elasticsearch-version}.zip[] and the tar.gz instance from link:https://artifacts.elastic.co/downloads/elasticsearch/elasticsearch-oss-{elasticsearch-version}.tar.gz[]
...
NoNodeAvailableException[None of the configured nodes are available: [{#transport#-1}{v1hSCl1dQou7b93zEVw0gA}{127.0.0.1}{127.0.0.1:9300}]]
        ...
        at com.jecstar.etm.launcher.LaunchEtmCommand.launch(Launcher.java:57)
