...
For the storage of the events {etm} relies on an link:http://www.elastic.co/downloads[Elasticsearch 5.2] or compatible cluster. Both the Java runtime, and Elasticsearch cluster are not part of {etm} and need to be installed, configured and managed separately.
...
When you aree planning to create a multi node cluster, make sure the nodes are as close to each other as possible. Network latency can have a negative impact on the processing performance. Optical network interfaces are the preferred way to go, but if they are not available to you make sure your servers are equipped with at least Gigabit network interfaces. 
...
----
wget https://www.jecstar.com/downloads/etm-{project-version}.tgz
wget https://www.jecstar.com/downloads/etm-{project-version}.tgz.sha512
...
[source,bash,subs=attributes+]
----
wget https://www.jecstar.com/downloads/etm-{project-version}.zip
wget https://www.jecstar.com/downloads/etm-{project-version}.zip.sha512
cat etm-{project-version}.tgz.sha512 | sha512sum -c ## <1>
