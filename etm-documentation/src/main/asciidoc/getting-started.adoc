...
{etm} does not require a specific Elasticsearch cluster name and can join any Elasticsearch cluster, but out of the box it is trying to connect to a cluster with the name 'Enterprise Telemetry Monitor'. {etm} indici and aliasses will be prefixed with abbreviation ''etm_''.
...
IMPORTANT: By default {etm} tries to connect to an Elasticsearch cluster with the name ''Enterprise Telemetry Monitor'' and a node at 127.0.0.1:9300. If you have left your Elasticsearch cluster name untouched you should change that name, or change the {etm} configuration to match the Elasticsearch configuration.
...
When everything is configured well, you can access {etm} with a browser by browsing to the following address link:http://127.0.0.1:8080/gui/search/[]. During installation an administrative account is created with the username ''admin'' and the password ''password''.
