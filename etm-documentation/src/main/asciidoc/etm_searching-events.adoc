...
This chain shows an incoming (received) http request, a sent ShoppingCardRequest and finally a received ShoppingCardRequest chronologically plotted on time. Hovering over one of the time bars show the application and endpoint information. As you can see the bottom time bar is filled for 87%. This indicates that 87% of the total transaction time is spent in that request. When you are experiencing performance issues the 'ShoppingCard (received)' event is a good place to start your investigation.
...
{etm} supports a limited sets of joins.
...
WARNING: Under the hood {etm} will execute multiple queries to make these joins possible.
