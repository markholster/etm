...
[source,bash,subs=attributes+]
----
docker run -p 8080:8080 -e "elasticsearch.connectAddresses=<my-elasticsearch-host>:9300" -e "elasticsearch.clusterName=elasticsearch" jecstar.com/etm/etm:{project-version}
----

Most of the properties mentioned in the <<Node configuration>> chapter can be passed as environment variables to the Docker container.
...
services:
  ...
  es-data-node-1:
    ...
    environment:
      ...
      node.ingest: 'false'
      xpack.security.enabled: 'false'
      discovery.zen.ping.unicast.hosts: es-master-node-1
    networks:
      - etm
    ulimits:
      nofile: 65536
  ...
  etm-node-1:
    ...
    environment:
      ...
      elasticsearch.clusetName: Enterprise Telemetry Monitor
      elasticsearch.connectAddresses: es-master-node-1:9300
    networks:
      - etm
    ports:
      - "8080:8080"            
