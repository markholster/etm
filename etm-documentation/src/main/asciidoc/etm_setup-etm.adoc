...
[source,bash,subs=attributes+]
----
docker run -p 8080:8080 -e "elasticsearch_connectAddresses=<my-elasticsearch-host>:9300" -e "elasticsearch_clusterName=elasticsearch" jecstar.com/etm/etm:{project-version}
----
...
services:
  ...
  etm-node-1:
    ...
    environment:
      ...
      elasticsearch_clusetName: Enterprise Telemetry Monitor
      elasticsearch_connectAddresses: es-master-node-1:9300
    networks:
      - etm
    ports:
      - "8080:8080"            
